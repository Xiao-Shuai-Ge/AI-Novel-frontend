  ...
  <div>
    ...
    >
      ...
      <div class="flex items-center ml-5 w-1/6"
           @click="navigateTo('/')"
      >
        <span class="font-semibold text-blue-500 text-xl" v-if="!isMobile">AI小说</span>
        <span class="font-semibold text-black text-xl" v-if="!isMobile">创作平台</span>
      </div>
      ...
      >
        <div class="flex items-center cursor-pointer ml-auto">
          <span class="text-base font-bold mr-2 text-gray-800">{{ userName }}</span>
...
onMounted(async () => {
  ...
  if (UserStore.isLogin()) {
    ...
    // 从服务器获取用户信息
    const data = await get_user_self();
